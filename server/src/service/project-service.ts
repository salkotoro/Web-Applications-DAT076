...
export class ProjectService {
    ...
    async updateProject(idProj: number, name?: string, description?: string, salary?: number, roles?: [], open?: boolean): Promise<Project | null> {
        const projectIndex = this.projects.findIndex(project => project.idProj === idProj);
    ...
    async removeUserFromProject(idProj: number, id: number): Promise<Project | null> {
        const project = this.projects.find(project => project.idProj === idProj);
        if (!project) return null;
